...
var driveName;
var serverTime;	
var cancelledButtonCliked = false;
	
	
// driver profile & getting all trip details data start here
	
function getdetail() {
driverProfile();

	                            
	xhr.open("GET", "http://localhost:8080/Getalldetails/" + todayTripId, true);
...
function processResponseToGetDetail() {
    ...
    if (xhr.readyState == 4 && xhr.status == 261) {                      
    ...
    timeSlotToCompare = arr[0].timeSlot;
...
var submitBtn3 = document.getElementById("canceltrip");
submitBtn3.addEventListener('click', function() {
    cancelledButtonCliked = true;
    recheckCount();
});
...
function startTrip() {
    ...
    xhrServerTime.open("GET","http://localhost:8080/serverTime",true);
    xhrServerTime.send();
    xhrServerTime.onreadystatechange = function(){
        if(xhrServerTime.readyState==4 && xhrServerTime.status==200){
            serverTime = JSON.parse(this.responseText);
			...
			if (timeSlotToCompare > serverTime) {   //10:30 , 10:15
				...
				if(difference_in_hours==0){
					if (difference_in_minutes <= 15) {
						...
						document.getElementById("wrongTimeSlot").innerHTML = "<P class = 'WrongserverTime'> ⓘ Please Start The Trip Before 15 Minutes</p>";
					}
				}
				else{
					document.getElementById("wrongTimeSlot").innerHTML = "<P class = 'WrongserverTime'> ⓘ Please Start The Trip Before 15 Minutes</p>";
...
var url = "http://localhost:8080/update/for/" + todayTripId;
...
//alert("hi");
	...
	xhr.open("GET", "http://localhost:8080/Getalldetails/" + todayTripId,false);
...
//			count = countBeforeStart;
		...
		return false;
		}else if(cancelledButtonCliked == true) {
            cancelTripFunction();
        }
...
function starttripforshow() {
	...
	var url = "http://localhost:8080/update/for/show/" + todayTripId;
...
function ongoingForTripCabInfo() {
	...
	xhrupdate.open("PUT",  "http://localhost:8080/updateme/Ongoing/" + todayTripId, true);
...
//trip cancellning starts here

function tripaftercancelling() {

    var xhrupdate = new XMLHttpRequest();
    xhrupdate.open("PUT", "http://localhost:8080/notification/"+cabNumber+"/"+todayTripId,true);

...
function onloadFunctionCalls(){
	 ...
	 interval = setInterval('driverNotification()', 1000);
	 driverProfile();
}
...
function driverNotification() {
	...
	xhttp.open("GET", "http://localhost:8080/notification/"+cabId, true);
	...
	function processNotification() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (this.responseText) {
				...
				// BELOW CODE FOR CAB BOOKED TIMESLOTS
...
function driverProfile() {
	...
	xhttp.open("GET", "http://localhost:8080/driverProfile/"+cabId, false);
